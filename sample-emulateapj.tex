...
Figure \ref{fig:steps} illustrates the general principle by showing the relationship between measured (uncorrected) $\rm H_{\alpha}$ luminosity and $\Psi$. The low and high redshift measurements are shown as blue triangles and red diamonds respectively. The assumption we make is that the scatter in the relation is driven by astrophysics, but that the slope in the average relation is simply a distance effect, which can be corrected for. Figure \ref{fig:hist} summarizes the results of the process in a histogram. The dashed red and black lines show the distributions in $H_{\alpha}$ luminosity prior to correction in the low and high redshift subsamples respectively, while the solid lines show the corrected luminosity for the subsamples. These are constrained to be the same between the two subsamples by the assumption that no evolution occurs between their redshifts. This assumption is reasonable, as the largest $\rm \delta z$ between subsamples is 0.035, and the redshift range within a given subsample is the same as the redshift range between the subsamples.
